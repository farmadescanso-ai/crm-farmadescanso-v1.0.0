/**
 * Script para completar datos faltantes (emails, direcciones completas) 
 * de las clínicas dentales insertadas
 */
...
function construirBusqueda(cliente) {
  ...
  const poblacion = cliente.Poblacion || 'Córdoba';
  
  // Limpiar nombre (quitar caracteres especiales al final)
  ...
  if (nombreLimpio && direccion && direccion !== 'Córdoba') {
    ...
    busquedas.push(`"${nombreLimpio}" clínica dental Córdoba contacto`);
  }
...
/**
 * Extrae información de un texto (email, teléfono, dirección)
 */
function extraerInformacion(texto, cliente) {
  ...
  if (emails && emails.length > 0) {
    ...
    const emailsFiltrados = emails.filter(email => {
      ...
      return !emailLower.includes('example.com') && 
             ...
             !emailLower.includes('gmail.com') && // Puedes ajustar esto
  ...
  if (campoVacio(cliente.Direccion) || cliente.Direccion === 'Córdoba') {
  ...
  // Buscar código postal de Córdoba (14xxx)
...
async function buscarConPuppeteer(page, busqueda) {
  try {
    ...
    const informacion = await page.evaluate(() => {
      ...
      // Extraer enlaces (pueden contener la web de la clínica)
      const enlaces = document.querySelectorAll('a[href]');
      enlaces.forEach(enlace => {
        const href = enlace.href;
        if (href && (href.includes('clinic') || href.includes('dental') || href.includes('odontolog'))) {
          info.enlaces.push(href);
        }
      });
...
async function buscarInformacionCompleta(page, cliente) {
  ...
  try {
    ...
    if (textoEncontrado) {
      ...
      if (info.Direccion && (campoVacio(cliente.Direccion) || cliente.Direccion === 'Córdoba')) {
...
async function completarDatosClinicas() {
  ...
  try {
    console.log('🚀 Iniciando completado de datos de clínicas dentales...');
    if (DRY_RUN) {
    ...
    // Obtener clínicas dentales con datos faltantes
    console.log('📊 Obteniendo clínicas dentales...');
    const todasLasClinicas = await crm.query(`
      SELECT id, Nombre_Razon_Social, Direccion, Poblacion, Movil, Email, CodigoPostal
      FROM clientes 
