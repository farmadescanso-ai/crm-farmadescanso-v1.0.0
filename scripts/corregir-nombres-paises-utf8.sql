-- Corregir nombres de países con caracteres especiales UTF-8
-- Asegurar que todos los nombres tengan tildes y caracteres especiales correctos

USE farmadescanso;

-- Asegurar que la tabla usa UTF-8
ALTER TABLE `paises` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- Actualizar nombres de países con caracteres especiales correctos
...
UPDATE `paises` SET `Nombre_pais` = 'EMIRATOS ÁRABES UNIDOS (Abu Dabi, Dubai, Sharya, Ayman, Umm al-Qaiwain, Ras al-Jaima y Fuyaira)' WHERE `Id_pais` = 'AE';
...
UPDATE `paises` SET `Nombre_pais` = 'FRANCIA (Incluidos los departamentos franceses de ultramar: Reunión, Guadalupe, Martinica y Guayana Francesa)' WHERE `Id_pais` = 'FR';
...
UPDATE `paises` SET `Nombre_pais` = 'PARAGUAY' WHERE `Id_pais` = 'PY';
...
SELECT `Id_pais`, `Nombre_pais` FROM `paises` WHERE `Nombre_pais` LIKE '%Á%' OR `Nombre_pais` LIKE '%É%' OR `Nombre_pais` LIKE '%Í%' OR `Nombre_pais` LIKE '%Ó%' OR `Nombre_pais` LIKE '%Ú%' OR `Nombre_pais` LIKE '%ñ%' OR `Nombre_pais` LIKE '%Ñ%' ORDER BY `Nombre_pais` LIMIT 20;
