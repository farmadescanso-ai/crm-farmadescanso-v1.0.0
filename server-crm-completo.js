...
// Función helper para parsear Roll
...
app.use((req, res, next) => {
  ...
  if (token) {
    ...
    if (decoded) {
      ...
      // Token válido, establecer req.user para compatibilidad
      req.user = decoded;
      req.comercialId = decoded.id;
      req.comercial = decoded;
      
      // También establecer en sesión para compatibilidad con código existente
    ...
    // Si no hay token pero hay sesión, parsear Roll en la sesión también
...
// Middleware para verificar autenticación (ahora usa JWT)
// Exportar requireAuth para uso en otros módulos
const requireAuth = (req, res, next) => {
  try {
    ...
    if (isRentabilidad) {
      ...
      console.log(`🔐 [AUTH] req.user:`, req.user ? 'existe' : 'ausente');
      console.log(`🔐 [AUTH] req.comercialId:`, req.comercialId || 'ausente');
      console.log(`🔐 [AUTH] req.session:`, req.session ? 'existe' : 'ausente');
      console.log(`🔐 [AUTH] req.session?.comercialId:`, req.session?.comercialId || 'ausente');
    }
    
    // Verificar JWT token primero (método principal)
    if (req.user && req.comercialId) {
      if (isRentabilidad) {
        console.log(`✅ [AUTH] Usuario autenticado (JWT): ${req.user.email || 'sin email'} (ID: ${req.comercialId})`);
      }
      next();
      return;
    }
    ...
    if (req.session && req.session.comercialId) {
      if (isRentabilidad) {
        console.log(`✅ [AUTH] Usuario autenticado (Sesión): ID ${req.session.comercialId}`);
      }
      next();
      return;
    }
    ...
    // No autenticado
    if (isRentabilidad) {
      console.log(`⚠️ [AUTH] No autenticado - Path: ${req.path}, Method: ${req.method}`);
      console.log(`⚠️ [AUTH] req.user:`, req.user ? 'presente' : 'ausente');
      console.log(`⚠️ [AUTH] req.comercialId:`, req.comercialId || 'ausente');
      console.log(`⚠️ [AUTH] req.session?.comercialId:`, req.session?.comercialId || 'ausente');
    }
    
    // Si es una petición fetch/AJAX, responder con JSON en lugar de redirect
    ...
    if (acceptsJson || isAjax || isFetch) {
      if (isRentabilidad) {
        console.log(`⚠️ [AUTH] Respondiendo con JSON 401 para petición fetch/AJAX`);
      }
...
const requireAdmin = (req, res, next) => {
  try {
    ...
    if (!req.user && !req.session?.comercialId) {
      console.log(`⚠️ [REQUIRE_ADMIN] No autenticado`);
    ...
    console.log(`✅ [REQUIRE_ADMIN] Usuario autenticado`);
    ...
    try {
      ...
      console.error('❌ [REQUIRE_ADMIN] Error verificando admin:', checkError.message);
      // Por seguridad, denegar acceso si hay error
      adminCheck = false;
    }
    
    if (!adminCheck) {
      console.log(`⚠️ [REQUIRE_ADMIN] Acceso denegado - no es admin`);
      if (req.path.startsWith('/api/')) {
    ...
    console.log(`✅ [REQUIRE_ADMIN] Acceso permitido`);
    next();
  } catch (error) {
    console.error('❌ [REQUIRE_ADMIN] Error en middleware:', error.message);
    console.error('❌ [REQUIRE_ADMIN] Stack:', error.stack);
    next(error);
  }
};
  ...
  try {
    ...
    if (!isPasswordValid) {
      ...
      return res.render('auth/login', {
        ...
        email: email || '',
        debugInfo: isDevelopment ? { paso: 4, error: 'Contraseña incorrecta' } : undefined
      });
    }
    ...
    if (!comercialId) {
      ...
      return res.render('auth/login', {
        ...
        email: email || '',
        debugInfo: isDevelopment ? { paso: 5, error: 'ID no válido' } : undefined
      });
    }
...
async function verifyPassword(password, hash) {
  // Si la contraseña está hasheada (empieza con $2a$, $2b$ o $2y$), usar bcrypt
...
app.get('/dashboard/clientes', requireAuth, async (req, res) => {
  ...
  try {
    ...
    let [provincias, tiposClientes, formasPago, idiomas, monedas, comerciales] = await Promise.all([
      ...
      crm.query('SELECT id, Idioma FROM idiomas').catch(() => []),
...
app.get('/dashboard/clientes/:id/editar', requireAuth, async (req, res) => {
  try {
    ...
    const [provincias, paises, cooperativasCliente, tiposClientes, formasPago, idiomas, monedas, comerciales] = await Promise.all([
      ...
      crm.query('SELECT id, Idioma FROM idiomas').catch(err => { console.error('Error obteniendo idiomas:', err); return []; }),
...
app.get('/dashboard/comerciales', requireAuth, requireAdmin, async (req, res) => {
  try {
    ...
    // Añadir nombre de provincia a cada comercial
    ...
    const comercialesConProvincia = comercialesArray.map(comercial => {
      ...
      // Inicializar nombreProvincia como cadena vacía por defecto
